...
fn test_state_context_clone() {
    ...
    ctx1.transition(StateTransition::Enable, Some("Test".to_string()))
        .unwrap();
    ctx1.record_failure();
    ctx1.record_failure();
...
fn test_state_transition_preserves_context() {
    ...
    ctx.transition(StateTransition::Enable, Some("Testing".to_string()))
        .unwrap();
...
fn test_recovery_abort_path() {
    ...
    ctx.transition(StateTransition::Enable, None).unwrap();
    ctx.transition(StateTransition::TriggerFallback, None)
        .unwrap();
    ctx.transition(StateTransition::StartRecovery, None)
        .unwrap();
