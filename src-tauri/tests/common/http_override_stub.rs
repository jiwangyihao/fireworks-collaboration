...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum IdempotentFlag {
    No,
    Yes,
}
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum MaxEventsCase {
    None,
    Some(u32),
}
...
impl Display for HttpOverrideCase {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "HttpOverride({:?},{:?},{:?},{:?})",
            self.op, self.follow, self.idempotent, self.max_events
        )
    }
}
...
impl CaseDescribe for HttpOverrideCase {
...
impl OverrideOutcome {
    ...
    pub fn terminal_event(&self) -> Option<&str> {
...
pub fn http_override_cases() -> Vec<HttpOverrideCase> {
    ...
    vec![
        HttpOverrideCase {
            ...
            idempotent: No,
            max_events: MaxEventsCase::None,
        },
        HttpOverrideCase {
            op: Clone,
            follow: FollowMode::Follow,
            idempotent: No,
            max_events: MaxEventsCase::Some(5),
        },
        HttpOverrideCase {
            op: Fetch,
            follow: FollowMode::Follow,
            idempotent: Yes,
            max_events: MaxEventsCase::Some(3),
        },
        HttpOverrideCase {
            op: Push,
            follow: FollowMode::None,
        ...
        HttpOverrideCase {
            ...
            follow: FollowMode::Follow,
            idempotent: Yes,
            max_events: MaxEventsCase::Some(2),
        },
    ]
}

/// 按操作类型过滤代表性 case（便于单文件内快速选择）
pub fn http_override_cases_for(op: GitOp) -> Vec<HttpOverrideCase> {
    http_override_cases()
        .into_iter()
        .filter(|c| c.op == op)
        .collect()
...
fn emit(events: &mut Vec<String>, e: impl Into<String>) {
    events.push(e.into());
}
...
pub fn run_http_override(case: &HttpOverrideCase) -> OverrideOutcome {
    ...
    if let MaxEventsCase::Some(0) = case.max_events {
        emit(&mut events, EV_INVALID_MAX);
...
mod tests_http_override_stub_smoke {
    ...
    fn smoke_http_override_cases_non_empty() {
        for c in http_override_cases() {
            ...
            assert_eq!(out.terminal_event(), Some("http:override:applied"));
        }
    }

    #[test]
    fn cases_filter_by_op() {
        use crate::common::git_scenarios::GitOp;
        assert!(http_override_cases_for(GitOp::Clone)
            .iter()
            .all(|c| matches!(c.op, GitOp::Clone)));
    }

    #[test]
    fn follow_chain_length_rule() {
        let with_follow = http_override_cases()
            .into_iter()
            .filter(|c| matches!(c.follow, FollowMode::Follow));
        for c in with_follow {
            let out = run_http_override(&c);
            let hops = follow_hops(&c);
            assert_eq!(out.follow_chain.len(), hops, "follow hops mismatch for {c}");
        }
    }
}
