...
impl PushRetryOutcome {
    pub fn is_success(&self) -> bool {
        matches!(self.result, PushResultKind::Success)
    }
    pub fn terminal_tag(&self) -> Option<&str> {
        self.events
            .iter()
            .rev()
            .find(|e| e.starts_with("push:result:"))
            .map(|s| s.as_str())
    }
}

#[derive(Debug, Clone, Copy)]
struct PushAttemptCtx {
    attempt: u8,
    delay: u64,
}
...
fn emit_attempt(events: &mut Vec<String>, ctx: PushAttemptCtx) {
    events.push(format!(
        "push:attempt#{}:delay={}ms",
        ctx.attempt, ctx.delay
    ));
}

fn build_clone_dest() -> PathBuf {
    std::env::temp_dir().join(format!("fwc-clone-dest-{}", uuid::Uuid::new_v4()))
}
...
pub fn run_push_with_retry(spec: &PushRetrySpec) -> PushRetryOutcome {
    ...
    if let PolicyOverride::AbortAfter(k) = spec.case.policy {
        if k > 0 {
            ...
            return PushRetryOutcome {
                ...
                attempts_used: k.saturating_sub(1),
                events,
            };
        }
    }
    ...
    for attempt in 1..=spec.case.attempts {
        ...
        let ctx = PushAttemptCtx {
            attempt,
            delay: seq_full[(attempt - 1) as usize],
        };
        ...
        if spec.simulate_conflict {
            if attempt == spec.case.attempts {
                ...
                events.push("push:result:retry".into());
                continue;
            }
        } else {
            result = PushResultKind::Success;
...
pub fn run_clone(params: &CloneParams) -> CloneOutcome {
    ...
    if let Some(f) = &params.filter {
        if f.is_empty() {
            events.push("filter:empty".into());
        }
    }
    if events.is_empty() {
        events.push("Init".into());
    }
...
pub fn assert_clone_events(label: &str, out: &CloneOutcome) {
    assert!(
        !out.events.is_empty(),
        "[{label}] events should not be empty"
    );
    ...
    fn push_retry_force_success_early_smoke() {
        ...
        let out = run_push_with_retry(&PushRetrySpec {
            case: RetryCase {
                ...
                base_delay_ms: case.base_delay_ms,
                backoff: case.backoff,
                policy: PolicyOverride::ForceSuccessEarly,
            },
            simulate_conflict: true,
        });
        ...
        assert!(
            out.attempts_used >= 2,
            "should succeed on or after attempt 2"
        );
    }
}
