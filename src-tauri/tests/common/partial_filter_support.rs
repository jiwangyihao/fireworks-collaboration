...
impl SupportLevel {
    pub fn is_supported(self) -> bool { matches!(self, Self::Supported) }
    pub fn is_unsupported(self) -> bool { matches!(self, Self::Unsupported) }
    pub fn is_invalid(self) -> bool { matches!(self, Self::Invalid) }
    pub fn is_degraded(self) -> bool { matches!(self, Self::DegradedPlaceholder) }
    /// 返回紧凑 code（便于日志/快照断言）
    pub fn code(self) -> &'static str { match self { Self::Supported => "S", Self::Unsupported => "U", Self::DegradedPlaceholder => "D", Self::Invalid => "I" } }
}

#[derive(Debug, Clone)]
pub struct PartialFilterOutcome { pub support: SupportLevel, pub depth: Option<u32>, pub has_filter_marker: bool }

impl PartialFilterOutcome {
    pub fn is_fallback(&self) -> bool { self.support.is_unsupported() }
    pub fn is_invalid(&self) -> bool { self.support.is_invalid() }
    pub fn is_degraded(&self) -> bool { self.support.is_degraded() }
    /// 统一文本描述（便于参数化测试名称/快照）
    pub fn describe(&self) -> String { format!("{}-d{}-m{}", self.support.code(), self.depth.unwrap_or(0), if self.has_filter_marker { 'y' } else { 'n' }) }
}
...
// ---- 内部：分类逻辑与辅助 ----
fn detect_filter_marker(events: &[String]) -> bool { events.iter().any(|e| e.to_ascii_lowercase().contains("filter:")) }

/// 判定支持级别（不含 marker 检测）。
fn compute_support_level(lower_label: &str, depth: Option<u32>, env_supported: bool) -> SupportLevel {
    let is_invalid = lower_label.contains("bad:") || lower_label.contains("invalid") || lower_label.contains("bad-filter") || lower_label.contains("bad:filter");
    if is_invalid { return SupportLevel::Invalid; }
    if lower_label.contains("unsupported") { return SupportLevel::Unsupported; }
    if lower_label.contains("+depth") || depth.is_some() { return SupportLevel::DegradedPlaceholder; }
    if env_supported || lower_label.starts_with("filter:") || lower_label.is_empty() { return SupportLevel::Supported; }
...
mod tests_pf_support {
    use super::*;
    fn make_events() -> Vec<String> { vec!["Start".into(), "Filter: event-only".into()] }

    #[test]
    fn assess_rules_basic() {
        let ev = make_events();
        let r = assess_partial_filter("filter:event-only", None, &ev); assert!(r.support.is_supported()); assert!(r.has_filter_marker);
        let r2 = assess_partial_filter("filter:unsupported-case", None, &ev); assert!(r2.is_fallback());
        let r3 = assess_partial_filter("filter:code+depth", Some(1), &ev); assert!(r3.is_degraded());
        let r4 = assess_partial_filter("filter:bad:filter", None, &ev); assert!(r4.is_invalid());
    }
    ...
    fn outcome_describe_shape() {
        ...
        assert!(d.starts_with("S-d1") || d.starts_with("D-d1"), "unexpected describe prefix: {d}");
    }
}
