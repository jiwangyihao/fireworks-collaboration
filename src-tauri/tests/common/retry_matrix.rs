...
pub fn retry_cases() -> Vec<RetryCase> {
    use BackoffKind::*; use PolicyOverride::*;
    vec![
        RetryCase { attempts: 1, base_delay_ms: 50,  backoff: Constant,    policy: None },
        RetryCase { attempts: 3, base_delay_ms: 100, backoff: Constant,    policy: None },
        RetryCase { attempts: 3, base_delay_ms: 80,  backoff: Linear,      policy: None },
        RetryCase { attempts: 3, base_delay_ms: 60,  backoff: Exponential, policy: None },
        RetryCase { attempts: 5, base_delay_ms: 40,  backoff: Exponential, policy: AbortAfter(3) },
        RetryCase { attempts: 5, base_delay_ms: 40,  backoff: Linear,      policy: ForceSuccessEarly },
    ]
}
...
pub fn compute_backoff_sequence(case: &RetryCase) -> Vec<u64> {
    let mut seq = Vec::new();
    for n in 0..case.attempts { // n 表示第 n 次尝试前的等待
