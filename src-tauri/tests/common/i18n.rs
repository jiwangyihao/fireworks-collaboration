...
pub fn locale_keys() -> Vec<&'static str> { fixture().keys().cloned().collect() }
...
mod tests {
    ...
    fn basic_translate_and_keys() {
        ...
        assert_eq!(translate("error.network.timeout", "en").as_deref(), Some("Network timeout"));
        assert_eq!(translate("error.network.timeout", "zh").as_deref(), Some("网络超时"));
        assert_eq!(translate("error.network.timeout", "fr").as_deref(), Some("Network timeout"));
