...
mod section_params_and_behavior {
    ...
    use crate::common::git_scenarios::{CloneParams, run_clone, _run_clone_with_cancel, assert_clone_events};

    #[derive(Debug, Clone, Copy)]
    struct Case { depth: Option<u32>, filter: Option<&'static str> }
    impl Case { fn label(&self) -> String { format!("depth={:?},filter={:?}", self.depth, self.filter) } }
    fn cases() -> Vec<Case> { vec![
        ...
        Case { depth: Some(0), filter: None }, // 非法 depth (占位实现仍只检查事件)
    ] }
    ...
    fn clone_cancel_early() {
        ...
        assert!(out.dest.exists(), "[clone-core cancel-early] dest should exist");
