...
mod section_branch_create {
    ...
    fn branch_valid_and_checkout_and_duplicate() {
        ...
        for (i, v) in valids.iter().enumerate() {
            ...
            git_branch(&dest, v, false, false, &flag, |p: ProgressPayload| {
                phases.push(p.phase.clone())
            })
            .unwrap();
            assert!(
                phases.last().unwrap().starts_with("Branched"),
                "[branch-create] expected Branched phase for {v}"
            );
            if i == 0 {
                git_checkout(&dest, v, false, &flag, |_p| {}).unwrap();
            }
        }
    ...
    fn branch_invalid_names_combined() {
        ...
        for bad in invalids {
            let err = helpers::branch_op(&dest, bad, false, &flag).unwrap_err();
    ...
    fn branch_creation_without_commit_force_and_nonforce() {
        ...
        for force in [false, true] {
            ...
            let err = helpers::branch_op(
                &dest,
                if force { "main" } else { "feature/a" },
                force,
                &flag,
            )
            .unwrap_err();
    ...
    fn branch_force_move_updates_ref_once() {
        ...
        assert_eq!(
            tgt, new_head,
            "[branch-create] force move should update ref"
        );
    }
}
...
mod section_checkout_basic {
    ...
    use fireworks_collaboration_lib::core::git::default_impl::{
        branch::git_branch, checkout::git_checkout, commit::git_commit, init::git_init,
    };
    use fireworks_collaboration_lib::core::git::errors::ErrorCategory;
    use std::sync::atomic::AtomicBool;
    ...
    mod helpers {
        use super::*;
        pub(super) fn checkout_op(
            dest: &std::path::Path,
            name: &str,
            create: bool,
            flag: &AtomicBool,
        ) -> Result<(), fireworks_collaboration_lib::core::git::errors::GitError> {
            git_checkout(dest, name, create, flag, |_p| {})
        }
    }
