...
use fireworks_collaboration_lib::core::git::default_impl::{
    refname::{validate_branch_name, validate_ref_name, validate_remote_name, validate_tag_name},
...
// ---- 公共辅助 ----
fn cat(err: GitError) -> ErrorCategory {
    match err {
...
fn expect_protocol(res: Result<(), GitError>) {
    match res {
        ...
        Err(GitError::Categorized { category, .. }) => {
...
mod section_tag_lightweight {
    use super::*;
    #[test]
    fn tag_lightweight_success() {
        let dest = repo_with_one_commit();
        ...
        git_tag(
            ...
            false,
            &flag,
            |p: ProgressPayload| phases.push(p.phase),
        )
        .unwrap();
    ...
    fn tag_without_commit_rejected() {
        let dest = create_empty_repo().path;
        let flag = AtomicBool::new(false);
    ...
    fn tag_lightweight_force_updates_ref_oid() {
        ...
        let orig = repo
            .find_reference("refs/tags/lw")
            .unwrap()
            .target()
            .unwrap();
        commit_files(&dest, &[("extra.txt", "x")], "feat: extra", false).unwrap();
        ...
        git_tag(
            ...
            true,
            &flag,
            |p: ProgressPayload| phases2.push(p.phase),
        )
        .unwrap();
    ...
    fn tag_lightweight_force_same_head_oid_unchanged() {
        ...
        let new_oid = repo2
            ...
            .target()
            .unwrap();
        assert_eq!(orig, new_oid);
    }
}

// ---------------- section_tag_annotated ----------------
mod section_tag_annotated {
    use super::*;
    #[test]
    ...
    fn tag_annotated_force_creates_new_object() {
        ...
        git_tag(
            ...
            false,
            &flag,
            |p: ProgressPayload| phases1.push(p.phase),
        )
        .unwrap();
        let repo = git2::Repository::open(&dest).unwrap();
        let first_obj = repo
            .find_reference("refs/tags/ann")
            .unwrap()
            .target()
            .unwrap();
        let mut phases2 = Vec::<String>::new();
        ...
        .unwrap();
        let repo2 = git2::Repository::open(&dest).unwrap();
        let second_obj = repo2
            .find_reference("refs/tags/ann")
            .unwrap()
            .target()
            .unwrap();
        assert_ne!(first_obj, second_obj);
    ...
    fn tag_annotated_force_same_message_retains_oid() {
        ...
        assert_eq!(
            ...
            "retag with same message should keep target"
        );
        assert_eq!(
            first_msg, second_msg,
            "retag with same message should keep message"
        );
    }
    #[test]
...
mod section_remote_lifecycle {
    ...
    fn remote_add_set_remove_success() {
        ...
        phases.clear();
        git_remote_remove(&dest, "origin", &flag, |p: ProgressPayload| {
            phases.push(p.phase)
        })
        .unwrap();
    ...
    fn remote_add_duplicate_rejected() {
        ...
        let e =
            git_remote_add(&dest, "dup", "https://example.com/a.git", &flag, |_p| {}).unwrap_err();
        assert!(matches!(cat(e), ErrorCategory::Protocol));
    }
    #[test]
    fn remote_set_nonexistent_rejected() {
        ...
        let flag = AtomicBool::new(false);
        let e =
            git_remote_set(&dest, "nope", "https://example.com/x.git", &flag, |_p| {}).unwrap_err();
        assert!(matches!(cat(e), ErrorCategory::Protocol));
    }
    #[test]
...
mod section_remote_validation {
    use super::*;
    #[test]
    fn remote_invalid_name_or_url_rejected() {
        let dest = repo_with_one_commit();
        let flag = AtomicBool::new(false);
        let e = git_remote_add(
            &dest,
            "bad name",
        ...
        assert!(matches!(cat(e), ErrorCategory::Protocol));
        let e2 =
            git_remote_add(&dest, "ok", "ftp://example.com/x.git", &flag, |_p| {}).unwrap_err();
    ...
    fn remote_cancelled() {
        ...
        let flag = AtomicBool::new(true);
        let e =
            git_remote_add(&dest, "r1", "https://example.com/r.git", &flag, |_p| {}).unwrap_err();
        assert!(matches!(cat(e), ErrorCategory::Cancel));
    }
    #[test]
    fn remote_set_cancelled() {
        ...
        let e =
            git_remote_set(&dest, "c1", "https://example.com/y.git", &cancel, |_p| {}).unwrap_err();
        assert!(matches!(cat(e), ErrorCategory::Cancel));
    }
    #[test]
    ...
    fn tag_invalid_name_rejected() {
        ...
        for bad in [
            ...
            assert!(
                matches!(cat(e), ErrorCategory::Protocol),
...
mod section_refname_rules {
    ...
    fn refname_invalid_set() {
        ...
        for c in cases {
            expect_protocol(validate_ref_name(c));
        }
    }
    #[test]
