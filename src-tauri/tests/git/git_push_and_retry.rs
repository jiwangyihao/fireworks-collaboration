...
mod section_push_basic {
    ...
    fn push_basic_success_placeholder() {
        ...
        let case = retry_cases()
            .into_iter()
            .find(|c| c.attempts == 1)
            .expect("have attempts=1 case");
        let spec = PushRetrySpec {
            case,
...
mod section_push_conflict {
    ...
    fn push_conflict_variants_behave_as_expected() {
        ...
        let pick_exhausted = || {
            retry_cases()
                .into_iter()
                .find(|c| c.attempts >= 3 && matches!(c.policy, PolicyOverride::None))
                .expect("have attempts>=3 none policy case")
        };
        let pick_force_success = || {
            retry_cases()
                .into_iter()
                .find(|c| matches!(c.policy, PolicyOverride::ForceSuccessEarly))
                .expect("force success early case")
        };
        let pick_abort = || {
            retry_cases()
                .into_iter()
                .find(|c| matches!(c.policy, PolicyOverride::AbortAfter(_)))
                .expect("abort case")
        };
        ...
        // Exhausted
        {
            ...
            assert!(
                out.events.iter().any(|e| e.contains("exhausted")),
                "missing exhausted event"
            );
            expect_tag_subseq_min(&out.events, &["Attempt", "Attempt"]);
            assert_terminal_exclusive(
                &out.events,
                "result:exhausted",
                &["result:success", "result:abort"],
            );
        }
        ...
        // ForceSuccessEarly
        {
            ...
            assert!(
                out.attempts_used >= 2,
                "should succeed by attempt 2 or later"
            );
        ...
        // AbortBeforeAttempt
        {
            ...
            assert!(
                out.events.iter().any(|e| e.contains("abort")),
                "missing abort event"
            );
            ...
            assert_terminal_exclusive(
                &out.events,
                "result:abort",
                &["result:success", "result:exhausted"],
            );
        }
    }
}
...
mod section_retry_event {
    ...
    fn event_attempt_and_result_subsequences() {
        ...
        for kind in kinds {
            let (case, simulate_conflict, raw_anchors, tag_anchors): (_, bool, &[&str], &[&str]) =
                match kind {
