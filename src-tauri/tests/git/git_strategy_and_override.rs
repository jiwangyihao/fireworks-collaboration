...
mod section_http_invalid_max { use super::*; #[test] fn http_invalid_max_zero_applied_false() { let case = HttpOverrideCase { op: GitOp::Clone, follow: FollowMode::None, idempotent: IdempotentFlag::No, max_events: MaxEventsCase::Some(0) }; let out = run_http_override(&case); assert!(!out.applied); // 使用 DSL 子序列断言替代 contains 脆弱断言
expect_subsequence(&out.events, &["http:override:invalid_max"]); expect_optional_tags_subsequence(&out.events, &["http"]); }}
...
mod section_strategy_summary_multiop {
    ...
    async fn fetch_summary_event_and_applied_codes() {
        ...
        let work = tempfile::tempdir().unwrap(); let work_repo = git2::Repository::init(work.path()).unwrap(); work_repo.remote("origin", origin_desc.path.to_string_lossy().as_ref()).unwrap();
        let reg = std::sync::Arc::new(TaskRegistry::new());
        let (id, tk) = reg.create(TaskKind::GitFetch { repo: origin_desc.path.to_string_lossy().to_string(), dest: work.path().to_string_lossy().to_string(), depth: None, filter: None, strategy_override: None });
        let ov = serde_json::json!({"retry": {"max": 4}});
        let h = reg.spawn_git_fetch_task_with_opts(Some(AppHandle {}), id, tk, origin_desc.path.to_string_lossy().to_string(), work.path().to_string_lossy().to_string(), None, None, None, Some(ov));
        let _ = h.await;
    ...
    async fn push_summary_event_with_independent_applied_events() {
        ...
        let origin_desc = crate::common::repo_factory::RepoBuilder::new().with_base_commit("o.txt", "one", "c1").build();
        let src_desc = crate::common::repo_factory::RepoBuilder::new().with_base_commit("b.txt", "two", "c1").build();
        let src_repo = git2::Repository::open(&src_desc.path).unwrap();
        src_repo.remote("origin", origin_desc.path.to_string_lossy().as_ref()).unwrap();
        let reg = std::sync::Arc::new(TaskRegistry::new());
        let (pid, ptk) = reg.create(TaskKind::GitPush { dest: src_desc.path.to_string_lossy().to_string(), remote: Some("origin".into()), refspecs: None, username: None, password: None, strategy_override: None });
        let ov = serde_json::json!({"http": {"followRedirects": false}, "tls": {"insecureSkipVerify": true}});
        let h = reg.spawn_git_push_task(Some(AppHandle {}), pid, ptk, src_desc.path.to_string_lossy().to_string(), Some("origin".into()), None, None, None, Some(ov));
        let _ = h.await;
    ...
    fn summary_event_no_override_for_fetch_and_push() {
        ...
        rt.block_on(async {
            ...
            let origin_desc = crate::common::repo_factory::RepoBuilder::new().with_base_commit("a.txt", "one", "c1").build();
            let work = tempfile::tempdir().unwrap(); let work_repo = git2::Repository::init(work.path()).unwrap(); work_repo.remote("origin", origin_desc.path.to_string_lossy().as_ref()).unwrap();
            let reg = std::sync::Arc::new(TaskRegistry::new());
            let (fid, ftk) = reg.create(TaskKind::GitFetch { repo: origin_desc.path.to_string_lossy().to_string(), dest: work.path().to_string_lossy().to_string(), depth: None, filter: None, strategy_override: None });
            let fh = reg.spawn_git_fetch_task_with_opts(Some(AppHandle {}), fid, ftk, origin_desc.path.to_string_lossy().to_string(), work.path().to_string_lossy().to_string(), None, None, None, None); let _ = fh.await;
            crate::common::event_assert::assert_no_applied_codes(&fid.to_string());

            // origin & src for push
            let origin2 = crate::common::repo_factory::RepoBuilder::new().with_base_commit("c.txt", "three", "c1").build();
            let src_desc = crate::common::repo_factory::RepoBuilder::new().with_base_commit("d.txt", "four", "c1").build();
            let src_repo = git2::Repository::open(&src_desc.path).unwrap(); src_repo.remote("origin", origin2.path.to_string_lossy().as_ref()).unwrap();
            let (pid, ptk) = reg.create(TaskKind::GitPush { dest: src_desc.path.to_string_lossy().to_string(), remote: Some("origin".into()), refspecs: None, username: None, password: None, strategy_override: None });
            let ph = reg.spawn_git_push_task(Some(AppHandle {}), pid, ptk, src_desc.path.to_string_lossy().to_string(), Some("origin".into()), None, None, None, None); let _ = ph.await;
    ...
    fn strategy_override_summary_multi_codes_only() {
        ...
        rt.block_on(async {
            ...
            let _ = set_global_event_bus(std::sync::Arc::new(MemoryEventBus::new()));
            let src_desc = crate::common::repo_factory::RepoBuilder::new().with_base_commit("readme.txt", "hi", "c1").build();
            let reg = std::sync::Arc::new(TaskRegistry::new());
            let dest_dir = tempfile::tempdir().unwrap();
            let (id, token) = reg.create(TaskKind::GitClone { repo: src_desc.path.to_string_lossy().to_string(), dest: dest_dir.path().to_string_lossy().to_string(), depth: None, filter: None, strategy_override: None });
            let override_json = serde_json::json!({"http": {"follow_redirects": false}, "retry": {"max": 5}});
            let h = reg.spawn_git_clone_task_with_opts(Some(AppHandle {}), id, token.clone(), src_desc.path.to_string_lossy().to_string(), dest_dir.path().to_string_lossy().to_string(), None, None, Some(override_json)); let _ = h.await;
...
mod section_override_no_conflict {
    ...
    fn no_conflict_http_tls_override() {
        std::env::set_var("FWC_STRATEGY_APPLIED_EVENTS", "1");
        let tmp_src = crate::common::repo_factory::RepoBuilder::new().with_base_commit("z.txt", "nc", "c1").build();
    ...
    fn push_tls_insecure_only_event_once() {
        let rt = tokio::runtime::Runtime::new().unwrap(); rt.block_on(async {
            let _ = set_global_event_bus(std::sync::Arc::new(MemoryEventBus::new()));
            let src_desc = crate::common::repo_factory::RepoBuilder::new().with_base_commit("f.txt", "1", "c1").build();
            let reg = std::sync::Arc::new(TaskRegistry::new()); let app = AppHandle; let work = tempfile::tempdir().unwrap();
            let (cid, ctk) = reg.create(TaskKind::GitClone { repo: src_desc.path.to_string_lossy().to_string(), dest: work.path().to_string_lossy().to_string(), depth: None, filter: None, strategy_override: None });
            let ch = reg.clone().spawn_git_clone_task_with_opts(Some(app.clone()), cid, ctk, src_desc.path.to_string_lossy().to_string(), work.path().to_string_lossy().to_string(), None, None, None); wait_done(&reg, cid).await; ch.await.unwrap();
...
mod section_override_empty_unknown {
    ...
    async fn clone_with_empty_strategy_object_success() {
        ...
        let origin = crate::common::repo_factory::RepoBuilder::new().with_base_commit("f.txt", "1", "c1").build();
    ...
    async fn push_with_only_unknown_field_success() {
        let reg = std::sync::Arc::new(TaskRegistry::new());
        let work_desc = crate::common::repo_factory::RepoBuilder::new().with_base_commit("a.txt", "1", "c1").build();
        let unknown = serde_json::json!({"foo": {"bar": 1}});
        let (id, token) = reg.create(TaskKind::GitPush { dest: work_desc.path.to_string_lossy().to_string(), remote: None, refspecs: None, username: None, password: None, strategy_override: Some(unknown.clone()) }); token.cancel();
        let h = reg.clone().spawn_git_push_task(None, id, token, work_desc.path.to_string_lossy().to_string(), None, None, None, None, Some(unknown)); h.await.unwrap(); let snap = reg.snapshot(&id).unwrap(); assert!(!matches!(snap.state, TaskState::Failed));
    }
}
...
mod section_override_invalid_inputs {
    ...
    async fn push_with_invalid_strategy_override_array_fails() {
        let reg = std::sync::Arc::new(TaskRegistry::new());
        let tmp_desc = crate::common::repo_factory::RepoBuilder::new().with_base_commit("a.txt", "1", "c1").build();
        let bad = serde_json::json!([1,2,3]); let (id, token) = reg.create(TaskKind::GitPush { dest: tmp_desc.path.to_string_lossy().to_string(), remote: None, refspecs: None, username: None, password: None, strategy_override: Some(bad.clone()) });
        let h = reg.spawn_git_push_task(None, id, token, tmp_desc.path.to_string_lossy().to_string(), None, None, None, None, Some(bad)); h.await.unwrap(); let snap = reg.snapshot(&id).unwrap(); assert!(matches!(snap.state, TaskState::Failed));
    }
    #[tokio::test]
    async fn clone_invalid_http_max_redirects_fails() {
        let reg = std::sync::Arc::new(TaskRegistry::new());
        let origin_desc = crate::common::repo_factory::RepoBuilder::new().with_base_commit("f.txt", "1", "init").build();
        let dest = tempfile::tempdir().unwrap(); let bad = serde_json::json!({"http": {"max_redirects": 999}});
        let (id, token) = reg.create(TaskKind::GitClone { repo: origin_desc.path.to_string_lossy().to_string(), dest: dest.path().to_string_lossy().to_string(), depth: None, filter: None, strategy_override: Some(bad.clone()) });
    let h = reg.spawn_git_clone_task_with_opts(None, id, token, origin_desc.path.to_string_lossy().to_string(), dest.path().to_string_lossy().to_string(), None, None, Some(bad)); h.await.unwrap(); let snap = reg.snapshot(&id).unwrap(); assert!(matches!(snap.state, TaskState::Failed));
    }
    #[tokio::test]
    async fn push_invalid_retry_factor_fails() {
        let reg = std::sync::Arc::new(TaskRegistry::new());
        let work_desc = crate::common::repo_factory::RepoBuilder::new().with_base_commit("r.txt", "1", "c1").build();
        let bad = serde_json::json!({"retry": {"factor": 50.0}}); let (id, token) = reg.create(TaskKind::GitPush { dest: work_desc.path.to_string_lossy().to_string(), remote: None, refspecs: None, username: None, password: None, strategy_override: Some(bad.clone()) });
    let h = reg.spawn_git_push_task(None, id, token, work_desc.path.to_string_lossy().to_string(), None, None, None, None, Some(bad)); h.await.unwrap(); let snap = reg.snapshot(&id).unwrap(); assert!(matches!(snap.state, TaskState::Failed));
    }
}
...
mod section_summary_gating {
    ...
    let origin = crate::common::repo_factory::RepoBuilder::new().with_base_commit("readme.txt", "hi", "c1").build();
    ...
    let origin2 = crate::common::repo_factory::RepoBuilder::new().with_base_commit("g.txt", "hi", "c1").build();
    ...
    async fn tls_override_summary_and_gating() { std::env::set_var("FWC_STRATEGY_APPLIED_EVENTS", "1"); let _ = set_global_event_bus(std::sync::Arc::new(MemoryEventBus::new()));
        let origin = crate::common::repo_factory::RepoBuilder::new().with_base_commit("f.txt", "one", "c1").build();
        ...
        std::env::set_var("FWC_STRATEGY_APPLIED_EVENTS", "0");
        let origin2 = crate::common::repo_factory::RepoBuilder::new().with_base_commit("y.txt", "one", "c1").build();
