...
fn test_unicode_in_all_fields() {
    ...
    for (host, username, password) in unicode_test_cases {
        ...
        store
            .add(cred)
            .unwrap_or_else(|_| panic!("应该能添加 Unicode 凭证: {host}"));

        let retrieved = store
            .get(host, Some(username))
            .expect("应该能查询")
            .expect("凭证应该存在");
...
fn test_special_characters_in_credentials() {
    ...
    for (i, special) in special_chars.iter().enumerate() {
        ...
        store
            .add(cred)
            .unwrap_or_else(|_| panic!("应该能添加特殊字符凭证: {special}"));

        let retrieved = store
            .get(&format!("host{i}.com"), Some(special))
            .expect("应该能查询")
            .expect("凭证应该存在");
...
fn test_whitespace_only_credentials() {
    ...
    for (i, ws) in whitespace_cases.iter().enumerate() {
        let cred = Credential::new(format!("host{i}.com"), ws.to_string(), ws.to_string());
...
fn test_encrypted_file_with_very_long_password() {
    ...
    store
        .set_master_password(long_master_password.clone())
        .expect("应该接受长主密码");
...
fn test_host_without_tld() {
    ...
    for host in hosts {
        ...
        store
            .add(cred)
            .unwrap_or_else(|_| panic!("应该能添加主机: {host}"));
...
fn test_case_sensitive_credentials() {
    ...
    for (host, username) in &test_cases {
        ...
        store
            .add(cred)
            .unwrap_or_else(|_| panic!("应该添加: {username}@{host}"));
    }
