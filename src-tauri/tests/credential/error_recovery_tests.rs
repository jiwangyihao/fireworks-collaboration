...
fn test_wrong_password_retry() {
    ...
    store
        .set_master_password("correct-password".to_string())
        .unwrap();
    ...
    // 使用错误密码
    store2
        .set_master_password("wrong-password".to_string())
        .unwrap();
...
fn test_disk_space_handling() {
    ...
    let cred = Credential::new("github.com".to_string(), "user".to_string(), huge_password);
...
fn test_multiple_wrong_password_attempts() {
    ...
    for i in 0..10 {
        ...
        store_temp
            .set_master_password(format!("wrong-password-{i}"))
            .unwrap();
