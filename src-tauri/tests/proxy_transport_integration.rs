...
fn test_transport_skipped_when_http_proxy_enabled() {
    ...
    assert!(result.is_ok(), "ensure_registered should succeed when proxy enabled");
}
...
fn test_transport_registered_when_proxy_off() {
    ...
    let result = ensure_registered(&cfg);
    assert!(result.is_ok(), "ensure_registered should succeed when proxy off");
}
...
fn test_transport_skipped_when_disable_custom_transport_set() {
    ...
    let result = ensure_registered(&cfg);
    assert!(result.is_ok(), "ensure_registered should succeed with disable_custom_transport");
}
...
fn test_transport_skipped_when_system_proxy_enabled() {
    ...
    let result = ensure_registered(&cfg);
    assert!(result.is_ok(), "ensure_registered should succeed with System proxy");
}
...
fn test_metrics_data_flow_with_proxy() {
    use fireworks_collaboration_lib::core::git::transport::metrics::{tl_reset, tl_set_proxy_usage, tl_snapshot};
    ...
    assert_eq!(snapshot.proxy_type, Some("http".to_string()), "proxy_type should be 'http'");
    assert_eq!(snapshot.proxy_latency_ms, Some(50), "proxy_latency_ms should be 50");
    assert_eq!(snapshot.custom_transport_disabled, Some(true), "custom_transport_disabled should be true");
    
    // Reset and verify clean state
    tl_reset();
    let snapshot_after_reset = tl_snapshot();
    assert_eq!(snapshot_after_reset.used_proxy, None, "used_proxy should be None after reset");
    assert_eq!(snapshot_after_reset.proxy_type, None, "proxy_type should be None after reset");
    assert_eq!(snapshot_after_reset.proxy_latency_ms, None, "proxy_latency_ms should be None after reset");
    assert_eq!(snapshot_after_reset.custom_transport_disabled, None, "custom_transport_disabled should be None after reset");
}
...
fn test_metrics_data_flow_without_proxy() {
    ...
    assert_eq!(snapshot.used_proxy, Some(false), "used_proxy should be false");
    assert_eq!(snapshot.proxy_type, None, "proxy_type should be None");
    assert_eq!(snapshot.proxy_latency_ms, None, "proxy_latency_ms should be None");
    assert_eq!(snapshot.custom_transport_disabled, Some(false), "custom_transport_disabled should be false");
}
...
fn test_empty_proxy_url_behavior() {
    ...
    let result = ensure_registered(&cfg);
    assert!(result.is_ok(), "ensure_registered should handle empty proxy URL gracefully");
    
    // ProxyManager should report as not enabled
    use fireworks_collaboration_lib::core::proxy::ProxyManager;
    let manager = ProxyManager::new(cfg.proxy.clone());
    assert!(!manager.is_enabled(), "Empty URL should mean proxy is not enabled");
}
