...
fn simulate_lifecycle(spec: &LifecycleSpec) -> LifecycleOutcome {
    ...
    match spec.variant {
        Success | Fail | CancelMid => {
            if matches!(spec.kind, Clone | Fetch | Push | Sleep) {
    ...
    match spec.variant {
        CancelEarly => {
            ev.push("cancel:requested:early".into());
            ev.push("task:end:cancelled".into());
        }
        CancelMid => {
            ev.push("cancel:requested:mid".into());
            ev.push("task:end:cancelled".into());
        }
        Fail => {
            ev.push("task:end:failed".into());
        }
        Success => {
            ev.push("task:end:completed".into());
        }
    }
    if spec.metrics {
        ev.push("metric:duration_ms:42".into());
        ev.push("metric:bytes:1024".into());
    }
...
fn expected_tag_sequence(variant: LifecycleVariant) -> &'static [&'static str] {
    use LifecycleVariant::*;
    match variant {
        Success | Fail => &["task", "task"],
        CancelEarly | CancelMid => &["task", "cancel", "task"],
    }
}
...
fn terminal_anchor(variant: LifecycleVariant) -> (&'static str, &'static [&'static str]) {
    ...
    match variant {
        Success => (
            "task:end:completed",
            &["task:end:failed", "task:end:cancelled"],
        ),
        Fail => (
            "task:end:failed",
            &["task:end:completed", "task:end:cancelled"],
        ),
        CancelEarly | CancelMid => (
            "task:end:cancelled",
            &["task:end:completed", "task:end:failed"],
        ),
    }
}
...
fn assert_lifecycle_core(spec: &LifecycleSpec) -> LifecycleOutcome {
    ...
    // 基础形状断言：首事件 & 终态互斥
    assert!(
        out.events.first().unwrap().starts_with("task:start:"),
        "first event must be task:start:*: {:?}",
        out.events.first().unwrap()
    );
...
fn lifecycle_core_matrix() {
    use GitTaskKind as K;
    use LifecycleVariant as V;
    ...
    for spec in matrix {
        let _ = assert_lifecycle_core(spec);
    }
}
...
fn lifecycle_metrics_success() {
    ...
    let metric_lines: Vec<&String> = out
        .events
        .iter()
        .filter(|e| e.starts_with("metric:"))
        .collect();
    assert!(
        metric_lines.len() >= 2,
    ...
    let end_pos = out
        .events
        .iter()
        .position(|e| e.contains("task:end:completed"))
        .unwrap();
    for m in metric_lines {
        let pos = out.events.iter().position(|e| e == m).unwrap();
        assert!(
            pos > end_pos,
            "metric event should appear after terminal event"
        );
    }
}
