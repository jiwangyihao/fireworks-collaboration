...
async fn fetch_no_filter_has_no_fallback_code() {
  ...
  handle.await.unwrap();
  let mut attempts=0; let mut seen=false; while attempts<8 { if let Some(bus)=get_global_memory_bus() { if bus.snapshot().iter().any(|e| matches!(e, Event::Transport(TransportEvent::PartialFilterFallback { .. }))) { seen=true; break; } } tokio::time::sleep(std::time::Duration::from_millis(40)).await; attempts+=1; }
  assert!(!seen, "should not emit structured fallback when filter absent");
}
