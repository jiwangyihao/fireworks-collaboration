...
fn strategy_override_http_tls_retry_combo_parallel() {
    ...
    rt.block_on(async {
        ...
        for (topic,p) in &ev { if topic=="task://error" { if p.contains(&id1.to_string()) { if p.contains("http_strategy_override_applied") { http1+=1; } if p.contains("tls_strategy_override_applied") { tls1+=1; } if p.contains("retry_strategy_override_applied") { retry1+=1; } } if p.contains(&id2.to_string()) { if p.contains("tls_strategy_override_applied") { tls2+=1; } if p.contains("http_strategy_override_applied") || p.contains("retry_strategy_override_applied") { panic!("unexpected http/retry event for task2"); } } if p.contains(&id3.to_string()) && p.contains("_strategy_override_applied") { panic!("no override events expected for task3"); } } }
