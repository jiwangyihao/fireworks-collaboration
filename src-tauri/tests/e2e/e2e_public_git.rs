...
use common::pipeline::{PipelineSpec, PipelineConfig, FaultKind, run_pipeline_with, assert_commit_growth_at_least, assert_commit_unchanged, assert_failure_commit_not_advanced};

#[ctor::ctor]
fn __init_env() { init_test_env(); }

#[derive(Debug, Clone, Copy)]
enum Scenario { FullSuccess, ReadOnly, PushFailure }
...
impl Scenario {
    ...
    fn config(self) -> PipelineConfig {
        match self {
            Scenario::FullSuccess => PipelineConfig { remote_commits: 2, enable_real: true, faults: vec![] },
            Scenario::ReadOnly => PipelineConfig { remote_commits: 3, enable_real: true, faults: vec![] },
            Scenario::PushFailure => PipelineConfig { remote_commits: 1, enable_real: true, faults: vec![FaultKind::ForcePushFailure] },
        }
    }
}
...
fn pipeline_scenarios_cover_success_readonly_and_failure() {
    for scenario in [Scenario::FullSuccess, Scenario::ReadOnly, Scenario::PushFailure] {
