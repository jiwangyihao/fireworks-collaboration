...
async fn filter_only_fallback_full_message() {
    ...
    handle.await.unwrap();
    // 轮询捕获事件（clone 线程已结束一般一次即可，保险重复）
    let mut found=false; let mut attempts=0; while attempts<20 && !found { let evs = peek_captured_events(); for (topic, json) in &evs { if topic=="task://error" && json.contains("fallback=full") && json.contains("GitClone") { found=true; break; } } if !found { tokio::time::sleep(std::time::Duration::from_millis(50)).await; } attempts+=1; }
    assert!(found, "expected fallback=full protocol error event for filter-only clone");
}
