...
impl ProxyConfig {
    ...
    fn validate_port(&self) -> anyhow::Result<()> {
        ...
        if let Some(url_after_scheme) = self
            .url
            .strip_prefix("http://")
    ...
    fn validate_probe_url(&self) -> anyhow::Result<()> {
        ...
        if let Some(colon_pos) = self.probe_url.rfind(':') {
            ...
            match port_str.parse::<u16>() {
                ...
                Err(_) => anyhow::bail!(
                    "probeUrl: Probe URL port is not a valid number: {}",
                    port_str
                ),
            }
        } else {
    ...
    fn validate_recovery_threshold(&self) -> anyhow::Result<()> {
        if self.recovery_consecutive_threshold == 0 {
            anyhow::bail!(
                "recoveryConsecutiveThreshold: Recovery consecutive threshold must be at least 1"
            );
        }

        if self.recovery_consecutive_threshold > 10 {
            anyhow::bail!(
                "recoveryConsecutiveThreshold: Recovery consecutive threshold must not exceed 10"
            );
        }
