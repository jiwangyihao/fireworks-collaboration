...
pub fn load_retry_plan() -> RetryPlan {
    match load_or_init() { Ok(cfg) => cfg.retry.into(), Err(_) => RetryPlan { max: 3, base_ms: 300, factor: 1.5, jitter: true } }
}

pub fn is_retryable(err: &GitError) -> bool {
    let (cat, msg) = match err { GitError::Categorized { category, message } => (*category, message.to_ascii_lowercase()) };
    match cat {
        ErrorCategory::Network => true,
        ErrorCategory::Protocol => {
            // 粗略判断是否为 5xx 类错误
            msg.contains(" 5") || msg.contains("http 5") || msg.contains(" 50") || msg.contains(" 51") || msg.contains(" 52") || msg.contains(" 53") || msg.contains(" 54") || msg.contains(" 55") || msg.contains(" 56") || msg.contains(" 57") || msg.contains(" 58") || msg.contains(" 59")
        }
        _ => false,
    }
}
...
pub fn backoff_delay_ms(plan: &RetryPlan, attempt_idx: u32) -> u64 {
    ...
    if plan.jitter {
        ...
        if low >= high { base } else { rand::thread_rng().gen_range(low..=high) }
    } else { base }
}
...
mod tests {
    ...
    fn test_backoff_monotonic_no_jitter() {
        let p = RetryPlan { max: 3, base_ms: 100, factor: 2.0, jitter: false };
    ...
    fn test_backoff_with_jitter_range() {
        let p = RetryPlan { max: 5, base_ms: 200, factor: 1.5, jitter: true };
