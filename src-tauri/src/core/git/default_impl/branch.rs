...
pub fn git_branch<F: FnMut(ProgressPayload)>(
    ...
    if let Some(br) = existing_branch {
        // Branch exists
        if force {
            if head_oid.is_none() { return Err(GitError::new(ErrorCategory::Protocol, "no commit to move branch to")); }
            if should_interrupt.load(Ordering::Relaxed) { return Err(GitError::new(ErrorCategory::Cancel, "user canceled")); }
            if let Some(oid) = head_oid { let commit = repo.find_commit(oid).map_err(|e| GitError::new(ErrorCategory::Internal, format!("find commit: {}", e.message())))?; br.into_reference().set_target(commit.id(), "force branch move").map_err(|e| GitError::new(ErrorCategory::Internal, format!("move branch: {}", e.message())))?; }
        } else if !checkout {
            return Err(GitError::new(ErrorCategory::Protocol, "branch already exists"));
        }
        if checkout {
            // perform checkout
            if should_interrupt.load(Ordering::Relaxed) { return Err(GitError::new(ErrorCategory::Cancel, "user canceled")); }
            ...
            co.safe();
            repo.set_head(&format!("refs/heads/{}", branch_name)).map_err(|e| GitError::new(ErrorCategory::Internal, format!("set head: {}", e.message())))?;
            repo.checkout_head(Some(&mut co)).map_err(|e| GitError::new(ErrorCategory::Internal, format!("checkout: {}", e.message())))?;
            on_progress(ProgressPayload { task_id: uuid::Uuid::nil(), kind: "GitBranch".into(), phase: "BranchedAndCheckedOut".into(), percent: 100, objects: None, bytes: None, total_hint: None });
        } else {
            on_progress(ProgressPayload { task_id: uuid::Uuid::nil(), kind: "GitBranch".into(), phase: "Branched".into(), percent: 100, objects: None, bytes: None, total_hint: None });
        }
        return Ok(());
    }

    // Branch does not exist -> need a HEAD commit to point at
    let head_oid = match head_oid { Some(oid) => oid, None => return Err(GitError::new(ErrorCategory::Protocol, "repository has no commits (cannot create branch)")) };
    if should_interrupt.load(Ordering::Relaxed) { return Err(GitError::new(ErrorCategory::Cancel, "user canceled")); }
    let commit = repo.find_commit(head_oid).map_err(|e| GitError::new(ErrorCategory::Internal, format!("find commit: {}", e.message())))?;
    repo.branch(branch_name, &commit, false).map_err(|e| GitError::new(ErrorCategory::Internal, format!("create branch: {}", e.message())))?;
    if checkout {
        if should_interrupt.load(Ordering::Relaxed) { return Err(GitError::new(ErrorCategory::Cancel, "user canceled")); }
