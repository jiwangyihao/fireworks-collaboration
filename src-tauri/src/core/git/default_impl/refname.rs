...
use super::super::errors::{ErrorCategory, GitError};
...
pub fn validate_ref_name(raw: &str) -> Result<(), GitError> {
    ...
    if name.contains(' ') {
        return err("name contains space");
    }
    if name.starts_with('/') {
        return err("name starts with '/'");
    }
    if name.contains("//") {
        return err("name contains '//'");
    }
    if name.starts_with('-') {
        return err("name starts with '-'");
    }
    if name.ends_with('/') || name.ends_with('.') {
        return err("name ends with invalid char");
    }
    if name.ends_with(".lock") {
        return err("name ends with .lock");
    }
    if name.contains("..") {
        return err("name contains '..'");
    }
    if name.contains('\\') {
        return err("name contains backslash");
    }
    // Illegal single characters we disallow (common problematic set)
    const ILLEGAL: &[char] = &[':', '?', '*', '[', '~', '^'];
    if name.chars().any(|c| ILLEGAL.contains(&c)) {
        return err("name contains illegal char");
    }
    if name.contains("@{") {
        return err("name contains '@{'");
    }
    if name.chars().any(|c| c.is_control()) {
        return err("name has control char");
    }
    Ok(())
}

#[inline]
fn err(msg: &str) -> Result<(), GitError> {
    Err(GitError::new(ErrorCategory::Protocol, msg))
}
