...
#[inline]
pub fn preflight_generic<F: FnMut(ProgressPayload)>(
    kind: &str,
...
#[inline]
pub fn emit_sni_status<F: FnMut(ProgressPayload)>(
    kind: &str,
    maybe_repo_url: Option<&str>,
    on_progress: &mut F,
) {
    let cfg = match load_or_init() {
        Ok(c) => c,
        Err(_) => return,
    };
    ...
    let (sni, used_fake) = match host_opt.as_deref() {
        ...
        None => decide_sni_host_with_proxy(&cfg, false, "github.com", proxy),
    };
...
#[inline]
pub fn percent(received: u64, total: u64) -> u32 {
    if total > 0 {
...
#[inline]
pub fn total_hint(total: u64) -> Option<u64> {
    if total > 0 {
        Some(total)
    } else {
        None
...
pub fn is_local_path_candidate(s: &str) -> bool {
    ...
    if p.is_absolute() || s.starts_with("./") || s.starts_with("../") || s.contains('\\') {
        return true;
    }
    if !s.contains("://") && p.exists() {
        return true;
    }
    false
}
