...
pub fn git_commit<F: FnMut(ProgressPayload)>(
    ...
    if msg.is_empty() { return Err(GitError::new(ErrorCategory::Protocol, "commit message is empty")); }
    ...
    let tree_id = index.write_tree().map_err(|e| GitError::new(ErrorCategory::Internal, format!("write tree: {}", e.message())))?;
    let tree = repo.find_tree(tree_id).map_err(|e| GitError::new(ErrorCategory::Internal, format!("find tree: {}", e.message())))?;

    // Detect empty commit (no changes) when not allowed
    let head_commit_opt = repo.head().ok().and_then(|h| h.target()).and_then(|oid| repo.find_commit(oid).ok());
    if !allow_empty {
        let is_empty = match &head_commit_opt {
            Some(head_commit) => {
                // Compare tree ids
                head_commit.tree().map(|t| t.id() == tree.id()).unwrap_or(false)
            },
            None => index.is_empty(), // first commit case: if index empty it's empty
        };
        if is_empty { return Err(GitError::new(ErrorCategory::Protocol, "empty commit (no changes)")); }
    }
    ...
    let sig = if let Some(a) = author {
        match (a.name, a.email) {
            (Some(n), Some(e)) if !n.trim().is_empty() && !e.trim().is_empty() => git2::Signature::now(n.trim(), e.trim())
                .map_err(|e| GitError::new(ErrorCategory::Internal, format!("signature: {}", e.message())))?,
            _ => return Err(GitError::new(ErrorCategory::Protocol, "author name/email required when author specified"))
        }
    } else {
        repo.signature().map_err(|e| GitError::new(ErrorCategory::Internal, format!("signature: {}", e.message())))?
    };
    ...
    let commit_id = repo.commit(Some("HEAD"), &sig, &sig, msg, &tree, &parent_refs)
    ...
    on_progress(ProgressPayload { task_id: uuid::Uuid::nil(), kind: "GitCommit".into(), phase: "Committed".into(), percent: 100, objects: None, bytes: None, total_hint: None });
