...
impl SniffingStream {
    ...
    fn ensure_request_sent(&mut self) -> std::io::Result<()> {
        match self.op {
            HttpOp::InfoRefs => {
                if !self.requested {
                    let path = format!("{}/info/refs?service=git-upload-pack", self.path);
    ...
    fn send_post(&mut self) -> std::io::Result<()> {
        let path = format!("{}/git-upload-pack", self.path);
        let host_hdr = if self.port == 443 {
            self.host.clone()
        } else {
            format!("{}:{}", self.host, self.port)
    ...
    fn parse_headers_and_setup(&mut self) -> std::io::Result<()> {
        ...
        loop {
            if let Some(pos) = find_double_crlf(&self.header_buf) {
                ...
                if matches!(self.op, HttpOp::InfoRefs) {
...
impl Write for SniffingStream {
    ...
    fn flush(&mut self) -> std::io::Result<()> {
        ...
        if let HttpOp::UploadPack = self.op {
            if !self.posted {
                self.send_post()?;
            }
        }
...
impl git2::transport::SmartSubtransport for CustomHttpsSubtransport {
    ...
    let (stream, used_fake_sni, sni_used) = self.connect_tls_with_fallback(host, port)?;
        ...
        let op = match _action {
            git2::transport::Service::UploadPackLs => HttpOp::InfoRefs,
            git2::transport::Service::UploadPack => HttpOp::UploadPack,
            other => {
                let _ = other; // avoid unused warning
                tracing::debug!(target="git.transport", host=%host, port=%port, "unsupported service");
                return Err(Error::from_str("unsupported service for https+custom"));
            }
        };
        // 包一层嗅探器：记录首部与状态
