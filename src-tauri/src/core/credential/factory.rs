...
mod tests {
    ...
    fn test_factory_file_storage_fallback() {
        // File storage with invalid path should fallback to memory
        let config = CredentialConfig {
    ...
    fn test_factory_system_storage_fallback() {
        // System storage may not be available in test environment
        let config = CredentialConfig {
        ...
        let store = CredentialStoreFactory::create(&config);
        // Should either succeed or fallback to file/memory
