...
mod tests {
    ...
    fn create_test_store() -> (EncryptedFileStore, TempDir) {
        ...
        let store = EncryptedFileStore::new(&config).unwrap();
        store.set_master_password("test_master_password_123".to_string()).unwrap();
    ...
    #[test]
    fn test_file_store_add_get_remove() {
        let (store, _temp_dir) = create_test_store();
        ...
        // Get
        let retrieved = store.get("github.com", Some("test_user")).unwrap();
        assert!(retrieved.is_some());
        let retrieved = retrieved.unwrap();
        assert_eq!(retrieved.host, "github.com");
        assert_eq!(retrieved.username, "test_user");
        assert_eq!(retrieved.password_or_token, "test_password");

        // Remove
        assert!(store.remove("github.com", "test_user").is_ok());

        // Verify removed
        let after_remove = store.get("github.com", Some("test_user")).unwrap();
        assert!(after_remove.is_none());
    }

    #[test]
    fn test_file_store_encryption_roundtrip() {
        let (store, _temp_dir) = create_test_store();

        let cred = Credential::new(
            "github.com".to_string(),
            "user123".to_string(),
            "secret_password_456".to_string(),
        );
        ...
        let file_content = fs::read_to_string(&store.file_path).unwrap();
        assert!(!file_content.contains("secret_password_456"));
        assert!(file_content.contains("ciphertext"));
        assert!(file_content.contains("hmac"));

        // Retrieve and verify
        let retrieved = store.get("github.com", Some("user123")).unwrap().unwrap();
        assert_eq!(retrieved.password_or_token, "secret_password_456");
    }
    ...
    fn test_file_store_list() {
        ...
        let creds = vec![
            Credential::new("github.com".to_string(), "user1".to_string(), "pass1".to_string()),
            Credential::new("gitlab.com".to_string(), "user2".to_string(), "pass2".to_string()),
        ];
    ...
    fn test_file_store_wrong_password() {
        ...
        let wrong_store = EncryptedFileStore::new(&config).unwrap();
        wrong_store.set_master_password("wrong_password".to_string()).unwrap();

        // Should fail to decrypt
        let result = wrong_store.get("github.com", Some("user"));
    ...
    fn test_key_caching() {
        ...
        // First operation - derives key
        let start = std::time::Instant::now();
        store.add(cred).unwrap();
        let first_duration = start.elapsed();

        // Second operation - uses cached key (should be faster)
        let start = std::time::Instant::now();
        store.get("github.com", Some("user")).unwrap();
        let second_duration = start.elapsed();

        // Note: This might not always be faster due to timing variance,
        // but the cache should at least work
        println!("First op: {:?}, Second op: {:?}", first_duration, second_duration);
    }
}
