...
describe("api/git clone 调用", () => {
  ...
  it("startGitClone 透传 strategyOverride", async () => {
    ...
    const id = await startGitClone("repo", "C:/tmp/repo", {
      strategyOverride: { http: { followRedirects: false, maxRedirects: 0 } },
    });
    expect(invoke).toHaveBeenCalledWith("git_clone", {
      repo: "repo",
      dest: "C:/tmp/repo",
  ...
  it("startGitClone 仅传 depth/filter (无 strategyOverride)", async () => {
    ...
    const id = await startGitClone("repoX", "C:/tmp/r2", {
      depth: 1,
      filter: "blob:none",
    });
  ...
  it("startGitFetch 透传 strategyOverride + depth/filter", async () => {
    ...
    const id = await startGitFetch("repo2", "C:/tmp/repo2", {
      depth: 1,
      filter: "blob:none",
      strategyOverride: { retry: { max: 2 } },
    });
    expect(invoke).toHaveBeenCalledWith("git_fetch", {
      repo: "repo2",
      dest: "C:/tmp/repo2",
  ...
  it("startGitPush 透传 strategyOverride", async () => {
    ...
    const id = await startGitPush({
      dest: "C:/tmp/repo3",
      strategyOverride: { http: { followRedirects: false } },
    });
    expect(invoke).toHaveBeenCalledWith("git_push", {
      dest: "C:/tmp/repo3",
      strategyOverride: { http: { followRedirects: false } },
  ...
  it("startGitPush 透传 http+tls+retry 全量 strategyOverride", async () => {
    ...
    const override = {
      http: { followRedirects: false, maxRedirects: 1 },
      retry: { max: 2, baseMs: 400, factor: 1.2, jitter: false },
    };
    const id = await startGitPush({
      dest: "C:/tmp/repo4",
  ...
  it("startGitFetch 组合 preset+depth+filter+strategyOverride", async () => {
    ...
    const id = await startGitFetch("r-fetch", "C:/tmp/fetch", {
      preset: "branches",
      depth: 2,
      filter: "blob:none",
      strategyOverride: {
        http: { followRedirects: true, maxRedirects: 2 },
        retry: { max: 3 },
      },
    });
  ...
  it("startGitFetch 仅 strategyOverride", async () => {
    ...
    const id = await startGitFetch("r-fetch2", "C:/tmp/fetch2", {
      strategyOverride: { http: { followRedirects: false } },
    });
  ...
  it("startGitClone depth+filter+strategyOverride 全量", async () => {
    ...
    const id = await startGitClone("r-clone", "C:/tmp/clone", {
      depth: 3,
      filter: "blob:none",
      strategyOverride: {
        http: { followRedirects: true },
        retry: { max: 1, jitter: false },
      },
    });
  ...
  it("startGitClone 空 strategyOverride 对象", async () => {
    ...
    const id = await startGitClone("r-clone-empty", "C:/tmp/cloneE", {
      strategyOverride: {},
    });
  ...
  it("startGitPush retry-only strategyOverride", async () => {
    ...
    const id = await startGitPush({
      dest: "C:/tmp/retry",
      strategyOverride: { retry: { max: 5, baseMs: 250 } },
    });
