...
describe('multiple applied override events update store each time', () => {
  ...
  it('http -> retry -> conflict updates code sequentially', async () => {
    ...
    errL.cb({ payload: { taskId:'Z', kind:'GitFetch', category:'Protocol', code:'strategy_override_conflict', message:'http conflict: followRedirects=false => force maxRedirects=0 (was 2)' } });
    expect(store.lastErrorById['Z'].code).toBe('strategy_override_conflict');
    disposeTaskEvents();
  });
});
