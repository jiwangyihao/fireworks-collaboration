<template>
  <div class="p-4 pt-16 space-y-4">
    <h2 class="text-xl font-bold">Git 克隆面板</h2>
    ...
    <div class="card bg-base-100 shadow-sm">
      <div class="card-body gap-3">
        <div class="flex gap-2 items-center">
          ...
          <button class="btn btn-primary btn-sm" :disabled="!repo || !dest || working" @click="start">开始克隆</button>
        </div>
        <div class="text-xs opacity-70">建议使用绝对路径，例如 C:/tmp/project</div>
      </div>
    </div>
          ...
          <tbody>
            <tr v-for="t in tasks.items" :key="t.id">
              ...
              <td class="w-64">
                ...
                <div class="text-xs opacity-70">{{ progressOf(t.id).phase || '-' }} {{ progressOf(t.id).percent }}%</div>
              </td>
              <td>
...
import { startGitClone, cancelTask, listTasks } from '../api/tasks';
