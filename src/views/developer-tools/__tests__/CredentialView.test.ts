...
vi.mock("../../../components/developer-tools/CredentialForm.vue", () => ({
  default: {
    name: "CredentialForm",
...
vi.mock("../../../components/developer-tools/CredentialList.vue", () => ({
  default: {
    name: "CredentialList",
...
vi.mock("../../../components/developer-tools/MasterPasswordDialog.vue", () => ({
  default: {
...
vi.mock("../../../components/developer-tools/ConfirmDialog.vue", () => ({
  default: {
    name: "ConfirmDialog",
    template:
      '<div class="mock-confirm-dialog" v-if="show"><button @click="$emit(\'confirm\')">确认</button><button @click="$emit(\'cancel\')">取消</button></div>',
  ...
  describe("过期凭证警告", () => {
    ...
    it("应该在已过期警告中显示清理按钮", async () => {
      ...
      const cleanupButton = wrapper
        .findAll("button")
        .find((btn) => btn.text().includes("清理过期凭证"));
  ...
  describe("添加凭证功能", () => {
    it("应该点击添加凭证按钮显示表单", async () => {
      ...
      const addButton = wrapper
        .findAll("button")
        .find((btn) => btn.text().includes("添加凭证"));
    ...
    it("应该点击取消添加按钮隐藏表单", async () => {
      ...
      const cancelButton = wrapper
        .findAll("button")
        .find((btn) => btn.text().includes("取消添加"));
  ...
  describe("导出审计日志功能", () => {
    it("应该显示导出审计日志按钮", async () => {
      ...
      const exportButton = wrapper
        .findAll("button")
        .find((btn) => btn.text().includes("导出审计日志"));
