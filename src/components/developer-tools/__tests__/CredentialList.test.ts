...
vi.mock("../ConfirmDialog.vue", () => ({
  default: {
    name: "ConfirmDialog",
    template:
      '<div class="mock-confirm-dialog" v-if="show"><button class="confirm-btn" @click="$emit(\'confirm\')">确认</button><button class="cancel-btn" @click="$emit(\'cancel\')">取消</button></div>',
