...
import NetworkPanel from "../observability/NetworkPanel.vue";
...
describe("NetworkPanel", () => {
  ...
  describe("error state", () => {
    it("shows error message when error without snapshot", () => {
      ...
      expect(wrapper.find(".network-panel__error").exists()).toBe(true);
      expect(wrapper.text()).toContain("API error");
      expect(wrapper.text()).toContain("加载网络指标失败");
    });
  });
  ...
  describe("content state", () => {
    it("shows content when snapshot exists", () => {
      ...
      expect(wrapper.find(".network-panel__content").exists()).toBe(true);
    });
    ...
    it("shows stale badge when data is cached", () => {
      ...
      expect(wrapper.find(".network-panel__badge").exists()).toBe(true);
    ...
    it("renders metric cards section", () => {
      ...
      expect(wrapper.find(".network-panel__cards").exists()).toBe(true);
    });

    it("renders fallback chart section", () => {
      const wrapper = mount(NetworkPanel, {
        props: {
          snapshot: { generatedAtMs: Date.now(), series: [] },
          loading: false,
      ...
      expect(wrapper.find(".network-panel__chart").exists()).toBe(true);
    });
  });
});
