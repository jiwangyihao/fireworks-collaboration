...
const retrySeries = computed(() =>
  getSeries(props.snapshot, "git_retry_total")
);
...
const failureRate = computed(() =>
  computeRate(failedTasks.value, totalTasks.value)
);
...
const chartSeries = computed<ChartSeries[]>(() => {
  return kindStats.value.map((stat) => ({
    ...
    points: aggregatedCounterSeries(
      groupedByKind.value[stat.kind]?.filter(
        (entry) => labelValue(entry, "state") === "completed"
      ) ?? []
    ),
    color: undefined,
  }));
...
<template>
  <div class="git-panel flex flex-col gap-4">
    ...
    <div
      ...
      class="git-panel__error rounded-xl border border-error/40 bg-error/10 px-4 py-6 text-error"
    >
      加载 Git 指标失败：{{ error }}
    </div>
    ...
    <div v-else class="git-panel__content flex flex-col gap-6">
      <div
        class="git-panel__meta flex items-center gap-3 text-xs text-base-content/60"
        v-if="snapshot"
      >
        <span>总任务：{{ totalTasksDisplay }}</span>
        <span
          v-if="stale"
          class="git-panel__badge rounded-full bg-warning/10 px-2 py-0.5 text-warning"
          >数据为缓存</span
        >
      </div>
      <div class="git-panel__cards grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        >
          <ul
            v-if="retryByCategory.length"
            class="git-panel__retry-list mt-2 divide-y divide-base-200/60 text-xs"
          >
            <li v-for="item in retryByCategory" :key="item.category">
              <span
                class="git-panel__retry-category truncate font-medium text-base-content/70"
                >{{ item.category }}</span
              >
              <span
                class="git-panel__retry-count font-mono text-base-content"
      ...
      <section
        class="git-panel__table flex flex-col gap-2"
        v-if="kindStats.length"
      >
        <header>
          <h4 class="text-sm font-semibold text-base-content/80">
            任务类型明细
