...
<template>
  <div class="alerts-panel flex flex-col gap-4">
    ...
    <div v-else-if="error && !snapshot" class="alerts-panel__error rounded-xl border border-error/40 bg-error/10 px-4 py-6 text-error">加载告警指标失败：{{ error }}</div>
    <EmptyState v-else-if="showEmpty" message="暂无告警数据" />
    <div v-else class="alerts-panel__content flex flex-col gap-6">
      <div class="alerts-panel__meta flex items-center gap-3 text-xs text-base-content/60" v-if="snapshot">
        <span>告警触发总次数：{{ formatNumber(totalAlerts) }}</span>
        <span v-if="stale" class="alerts-panel__badge rounded-full bg-warning/10 px-2 py-0.5 text-warning">数据为缓存</span>
      </div>
      ...
      <section class="alerts-panel__chart flex flex-col gap-2">
        <header>
          <h4 class="text-sm font-semibold text-base-content/80">按严重度统计</h4>
        </header>
        <MetricChart :series="chartSeries" empty-message="暂无告警事件" :value-formatter="formatNumber" />
      </section>
      <section class="alerts-panel__table flex flex-col gap-2" v-if="tableRows.length">
        <header>
        ...
        <table class="w-full table-auto overflow-hidden rounded-xl border border-base-200 text-sm">
          ...
          <tbody>
            <tr v-for="row in tableRows" :key="row.severity" class="even:bg-base-200/20">
