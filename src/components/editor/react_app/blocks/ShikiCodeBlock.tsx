...
export const ShikiCodeBlockContent = (props: any) => {
  const {
    code,
    language,
    filename,
        ...
        <div className="relative z-20" ref={langButtonRef}>
          ...
          >
            <span className="font-mono">
              {LANGUAGES.find((l) => l.value === localLang)?.label || "Text"}
            </span>
            <Icon icon="lucide:chevron-down" className="w-3 h-3 opacity-50" />
          </div>
          <DropdownMenu
            isOpen={showLangMenu}
      ...
      >
        <CodeMirror
          ...
          onFocus={() => {
            ...
            contentRegistry.focusBlock(props.editor, props.block.id, "start");
          }}
          basicSetup={{
...
export const ShikiCodeBlock = createReactBlockSpec(
  ...
  {
    render: (props) => <ShikiCodeBlockContent {...props} />,
  }
);
